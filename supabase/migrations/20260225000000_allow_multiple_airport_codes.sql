-- Update airport_code validation to allow up to 3 comma-separated codes
-- e.g. "JFK", "JFK,LGA", or "SFO,OAK,SJC"
...
BEGIN
  ...
  -- Airport code: optional, 1-3 codes separated by commas (e.g. "JFK" or "JFK,LGA,EWR")
  IF NEW.airport_code IS NOT NULL AND NEW.airport_code !~ '^[A-Z]{3,4}(,[A-Z]{3,4}){0,2}$' THEN
    RAISE EXCEPTION 'Airport code must be 1-3 uppercase IATA codes separated by commas';
  END IF;
